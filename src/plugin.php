...
class WP_Font_Awesome extends WPS_Scripts {
	...
	/**
	 * Determines whether to add Font Awesome More.
	 *
	 * @since 1.0.0
	 *
	 * @var string
    ...
    public function register_more_styles() {
		...
		foreach ( array( 'font-awesome-corp', 'font-awesome-ext', 'font-awesome-social', ) as $handle ) {
			$args = $this->get_args( $handle, array( 'src' => plugins_url( 'lib/font-awesome-more/css/' . $handle . '.css' ), __FILE__  ) );
			if ( false !== $args && is_array( $args ) )
				wp_register_style( $args['handle'], $args['src'], $args['deps'], $args['ver'], $args['media'] );
		}
		
		$args = $this->get_args( 'font-awesome-more-ie7' );
		if ( false !== $args && is_array( $args ) ) {
			wp_register_style( $args['handle'], $args['src'], $args['deps'], $args['ver'], $args['media'] );
     ...
     */
	private function get_args( $handle, $args = array() ) {
		$this->handles[] = $handle;
		
		$defaults = apply_filters(
			'wp_font_awesome_default_args',
			array(
				'handle' => $handle,
				'src'    => plugins_url( 'lib/font-awesome/css/' . $this->suffix( $handle ), __FILE__  ),
				'deps'   => array(),
				'ver'    => '3.0',
				'media'  => 'all',
			),
		...
		return apply_filters( 'wp_font_awesome_args', $args, $handle );
	}
